...
const commands = {
    ...
    async neko(message) {
        ...
        try {
            ...
            await temp.edit(`Order up! ?????, One neko comming right up! `)
    ...
    async ping(message) {
        ...
        await tempMessage.edit(`?? Pong! Latency is ${latency}ms`);
    },
...
*Synopsis:* ${anime.synopsis}
    ...
    async creator(message) {
        await message.reply(`*Welcome to Yuki, A cute and personal Fun one!™?* \nProject maintained and fixed by Kushi_k \n©? Astral Axis 2024`);
    },
    ...
    async bedwars(message) {
        ...
        try {
            ...
            const temp = await message.reply('?? Fetching stats... Please wait! ?');
            ...
            let responseMessage = `?? *Bedwars Stats for ${playerName}* ??\n\n`;
